...
const VerTutores = () => {
    ...
    const fetchTutores = useCallback(async () => {
        ...
        } catch (error) { Swal.fire('Error', 'No se pudieron cargar los tutores.', 'error'); } 
    ...
    useEffect(() => {
        ...
        if (term) { filtered = filtered.filter(t => t.name.toLowerCase().includes(term) || (t.email && t.email.toLowerCase().includes(term)) || (t.dni && t.dni.includes(term)) || (t.phone && t.phone.includes(term))); }
        filtered.sort((a, b) => {
    ...
    const handleDelete = async (tutorId, tutorName) => {
       const result = await Swal.fire({ title: `¿Eliminar a ${tutorName}?`, text: "Esta acción no se puede deshacer. Los pacientes asociados NO serán eliminados.", icon: 'warning', showCancelButton: true, confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar' });
        if (result.isConfirmed) {
            try {
                await deleteDoc(doc(db, 'tutores', tutorId));
                Swal.fire('Eliminado', `${tutorName} ha sido eliminado.`, 'success');
                fetchTutores();
            } catch (error) { Swal.fire('Error', `No se pudo eliminar a ${tutorName}.`, 'error'); }
        }
    };
    ...
    return (
        <div className="presential-container">
            <div className="page-header"><h1>Gestión de Tutores</h1><Link to="/admin/add-tutor" className="btn btn-primary"><FaPlus /> Agregar Tutor</Link></div>
            <div className="filter-bar">
                <div className="filter-group"><input type="text" placeholder="Buscar por nombre, DNI, email o teléfono..." name="searchTerm" value={filters.searchTerm} onChange={handleFilterChange} /></div>
                 <div className="filter-group"><select name="sortOrder" value={filters.sortOrder} onChange={handleFilterChange}><option value="name_asc">Nombre (A-Z)</option><option value="name_desc">Nombre (Z-A)</option><option value="newest">Más nuevos</option></select></div>
