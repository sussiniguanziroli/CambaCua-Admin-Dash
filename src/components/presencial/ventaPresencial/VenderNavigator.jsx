...
const VenderNavigator = () => {
    ...
    const [saleData, setSaleData] = useState({ cart: [], tutor: null, patient: null, payments: [], debt: 0, total: 0, clinicalHistoryItems: [] });
    ...
    const handleToggleClinicalHistoryItem = (itemId) => {
        setSaleData((prev) => {
            const newItems = prev.clinicalHistoryItems.includes(itemId) ? prev.clinicalHistoryItems.filter((id) => id !== itemId) : [...prev.clinicalHistoryItems, itemId];
        ...
        try {
            ...
            batch.set(saleRef, {
                ...
                items: saleData.cart.map((item) => ({
                    ...
                    source: item.source, tipo: item.tipo, isDoseable: item.isDoseable || false, unit: item.unit || null,
                })),
            });
            ...
            if (itemsForHistory.length > 0 && saleData.patient?.id) {
                const treatmentText = itemsForHistory.map((s) => s.isDoseable ? `${s.name} (${s.quantity} ${s.unit})` : `${s.quantity} x ${s.name}`).join(", ");
                const clinicalNoteRef = doc(collection(db, `pacientes/${saleData.patient.id}/clinical_history`));
                batch.set(clinicalNoteRef, { createdAt: saleTimestamp, reason: "Productos/servicios de venta", treatment: treatmentText, saleId: saleRef.id });
            }
            ...
            Object.keys(schedule).forEach((itemId) => {
                ...
                if (days > 0 && saleData.patient?.id) {
                    ...
                    batch.set(vencimientoRef, {
                        productId: item.id, productName: item.name, tutorId: saleData.tutor.id, tutorName: saleData.tutor.name,
