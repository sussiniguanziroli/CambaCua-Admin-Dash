...
const SeleccionarProducto = ({ onProductsSelected, prevStep, initialCart, saleData, onSaleDateChange }) => {
    ...
    const isCacheValid = useCallback(() => {
        return productsCache && categoriesCache && serviceCategoriesCache && cacheTimestamp && (Date.now() - cacheTimestamp < CACHE_DURATION);
    }, []);
        ...
        try {
            ...
            const onlineData = onlineSnap.docs.map(doc => {
                ...
                return {
                    ...
                    isDoseable: data.isDoseable || data.esDosificable || false,
                };
            });
    ...
    const handleFilterChange = useCallback((e) => {
        ...
        setFilters(prev => {
            ...
            if (name === 'tipo') newFilters.category = 'todas';
            return newFilters;
        });
    }, []);
    ...
    const handleConfirmDose = useCallback((dose) => {
        ...
        setCart(prev => [...prev, {
            ...itemToDose,
            name: itemToDose.displayName,
            quantity: dose,
            unit: 'ml',
            id: `${itemToDose.id}-${Date.now()}`,
            originalProductId: itemToDose.id,
    ...
    const handleUpdateProductPrice = useCallback((itemId, newUnitPrice) => {
        ...
        const originalId = itemId.includes('-') ? (allProducts.find(p => itemId.startsWith(p.id))?.id || itemId) : itemId;
    ...
    const handleChangeSaleDate = useCallback(() => {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                if (!dateInput || !dateInput.value) { Swal.showValidationMessage('Por favor seleccione una fecha'); return false; }
            ...
            if (result.isConfirmed && result.value) {
                ...
                Swal.fire('Fecha Actualizada', `La fecha de la venta se ha establecido a ${selectedDate.toLocaleDateString('es-AR')}.`, 'success');
            }
        });
            ...
            <div className="seleccionar-producto-layout">
                <div className="producto-browser-panel">
                    ...
                    <div className="producto-list">
                        {isLoading ? <p>Cargando productos...</p> : filteredAndSortedData.map(item => (
                            <ProductListItem key={item.id} item={item} isInCart={!item.isDoseable && cart.some(cartItem => cartItem.id === item.id)} onAddToCart={handleAddToCart} />
                        ))}
                    </div>
                </div>
                <div className="smart-cart-panel">
                    ...
                    <div className="smart-cart-body">
                        {cart.length === 0 ? (
                            <div className="cart-empty-state"><CartIcon/><p>Tu carrito está vacío</p><span>Agrega productos</span></div>
                        ) : (
...
const ProductListItem = React.memo(({ item, isInCart, onAddToCart }) => {
    const handleClick = useCallback(() => { onAddToCart(item); }, [item, onAddToCart]);
    ...
    return (
        <div className={`producto-list-item ${isInCart ? 'in-cart' : ''}`}>
            ...
            <div className="item-actions">
                ...
                <button className="add-item-btn" onClick={handleClick} disabled={isInCart}>
                    {isInCart ? <CheckIcon/> : '+'}
                </button>
            </div>
        </div>
...
const CartItem = React.memo(({ item, onChangeQuantity, onOpenPriceModal, onRemove }) => {
    const handleDecrease = useCallback(() => { onChangeQuantity(item.id, item.quantity - 1); }, [item.id, item.quantity, onChangeQuantity]);
    const handleIncrease = useCallback(() => { onChangeQuantity(item.id, item.quantity + 1); }, [item.id, item.quantity, onChangeQuantity]);
    const handlePriceClick = useCallback(() => { onOpenPriceModal(item); }, [item, onOpenPriceModal]);
    ...
    return (
        <div className="cart-item-card">
            ...
            <div className="card-item-controls">
                ...
                <span className={`card-item-subtotal clickable-price ${item.discountAmount > 0 ? 'is-discounted' : ''}`} onClick={handlePriceClick} title="Click para modificar precio/descuento">
                    ${(item.price).toFixed(2)}
                </span>
                <button className="remove-item-btn" onClick={handleRemove}><TrashIcon/></button>
            </div>
        </div>
    );
});
