...
const SeleccionarProducto = ({ onProductsSelected, prevStep, initialCart, saleData }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        } catch (error) { Swal.fire('Error', 'No se pudieron cargar los productos.', 'error'); } finally { setIsLoading(false); }
    }, []);
    ...
    const handleViewChange = (mode) => { setViewMode(mode); setFilters({ text: '', category: 'todas', status: 'true', tipo: 'todos' }); };
    ...
    const handleConfirmDose = (dose) => {
        ...
        setCart(prev => [...prev, { ...itemToDose, quantity: dose, price: finalPrice, unit: 'ml', id: `${itemToDose.id}-${Date.now()}` }]);
        ...
        <div className="seleccionar-producto-container">
            ...
            <div className="seleccionar-producto-layout">
                ...
                <div className="smart-cart-panel">
                    ...
                    <div className="smart-cart-body">{cart.length === 0 ? (<div className="cart-empty-state"><CartIcon/><p>Tu carrito está vacío</p><span>Agrega productos</span></div>) : (<div className="cart-items-list">{cart.map(item => (<div key={item.id} className="cart-item-card"><div className="card-item-info"><span className="card-item-name">{item.name}</span><span className="card-item-price-unit">{item.unit === 'ml' ? `${item.quantity} ml` : `${item.quantity} u. x $${(item.originalPrice || 0).toFixed(2)}`}</span></div><div className="card-item-controls">{!item.isDoseable && (<div className="quantity-stepper"><button onClick={() => changeQuantity(item.id, item.quantity - 1)}>-</button><span>{item.quantity}</span><button onClick={() => changeQuantity(item.id, item.quantity + 1)}>+</button></div>)}<span className="card-item-subtotal">${(item.price).toFixed(2)}</span><button className="remove-item-btn" onClick={() => removeFromCart(item.id)}><TrashIcon/></button></div></div>))}</div>)}</div>
                    {cart.length > 0 && (<div className="cart-summary-footer"><div className="summary-line"><span>Subtotal</span><span>${total.toFixed(2)}</span></div><div className="summary-line total"><span>Total</span><strong>${total.toFixed(2)}</strong></div></div>)}
                </div>
            </div>
            <div className="seleccionar-producto-footer"><div className="venta-context-info"><span><strong>Tutor:</strong> {saleData.tutor?.name || 'Cliente Genérico'}</span><span><strong>Paciente:</strong> {saleData.patient?.name || 'N/A'}</span></div><div className="navigator-buttons"><button onClick={prevStep} className="btn btn-secondary">Anterior</button><button onClick={() => onProductsSelected(cart.map(({originalPrice, ...rest}) => rest), total)} className="btn btn-primary" disabled={cart.length === 0}>Siguiente</button></div></div>
        </div>
    );
};
