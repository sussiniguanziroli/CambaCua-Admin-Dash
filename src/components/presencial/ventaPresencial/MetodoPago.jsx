...
const MetodoPago = ({ onPaymentSelected, prevStep, saleData }) => {
    ...
    const totalSurcharges = useMemo(() => payments.reduce((acc, p) => acc + (p.surchargeAmount || 0), 0), [payments]);
    ...
    const addPaymentMethod = (method) => {
        const p = { id: Date.now(), method, amount: '' };
        if (method === 'Tarjeta de Crédito' || method === 'Tarjeta de Débito') { p.isCard = true; p.cardType = null; }
        setPayments(prev => [...prev, p]);
    };

    const updatePaymentAmount = (id, amount) => setPayments(prev => prev.map(p => p.id === id ? { ...p, amount } : p));

    const updateCardType = (id, cardType) => {
        setPayments(prev => {
            const current = prev.find(p => p.id === id);
            if (!current) return prev;
            const otherTotal = prev.filter(p => p.id !== id).reduce((acc, p) => acc + parseFloat(p.amount || 0), 0);
            const { percent, amount: newSurcharge } = calculateSurcharge(baseTotal, cardType);
            const required = baseTotal + newSurcharge - otherTotal;
            return prev.map(p => p.id === id ? { ...p, cardType, surchargePercent: percent, surchargeAmount: newSurcharge, amount: required > 0 ? required.toFixed(2) : '0.00' } : p);
        });
    };
    ...
    const handleNext = () => {
        let finalPayments = payments.filter(p => parseFloat(p.amount || 0) !== 0).map(p => ({
            id: p.id, method: p.method, amount: parseFloat(p.amount), date: saleData.saleTimestamp,
            cardType: p.cardType || null, surchargeAmount: p.surchargeAmount || 0, surchargePercent: p.surchargePercent || 0,
        }));
        let finalDebt = remainingBalance;
        if (remainingBalance < -0.01) {
            finalPayments.push({ id: `vuelto-${Date.now()}`, method: 'Efectivo', amount: -Math.abs(remainingBalance), date: saleData.saleTimestamp, isVuelto: true });
            finalDebt = 0;
        }
        if (finalDebt < 0.01) finalDebt = 0;
                                    ...
                                    {payment.isCard && payment.method === 'Tarjeta de Crédito' && (
                                        <div className="card-details">
                                            ...
                                            {payment.surchargeAmount > 0 && (
                                                <div className="surcharge-info">Recargo ({payment.surchargePercent}%): <span>+${payment.surchargeAmount.toFixed(2)}</span></div>
                                            )}
                                        </div>
                                    )}
                                </div>
