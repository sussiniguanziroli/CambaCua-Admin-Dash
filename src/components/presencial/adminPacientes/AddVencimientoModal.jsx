...
const AddVencimientoModal = ({ isOpen, onClose, onSave, pacienteId, tutorId, tutorName, pacienteName }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!selectedProduct || !dueDate) {
            setError('Debe seleccionar un producto y una fecha.');
            return;
        }
        setIsSubmitting(true);
        setError('');
        try {
            await addDoc(collection(db, `pacientes/${pacienteId}/vencimientos`), {
        ...
        <div className="agenda-modal-overlay">
            <div className="agenda-modal-content vencimiento-modal expanded">
                <div className="modal-header">
                    <h3>Agregar Vencimiento Manual</h3>
                    <button className="close-btn" onClick={handleClose}>&times;</button>
                </div>
                <form onSubmit={handleSubmit} className="vencimiento-form-layout">
                    <div className="vencimiento-product-selector">
                        <div className="vencimiento-view-switcher">
                            <button type="button" onClick={() => handleViewChange('presential')} className={viewMode === 'presential' ? 'active' : ''}>Items Presenciales</button>
                            <button type="button" onClick={() => handleViewChange('online')} className={viewMode === 'online' ? 'active' : ''}>Productos Online</button>
                        </div>
                        <div className="vencimiento-filters">
                            ...
                            {viewMode === 'presential' ? (
                                <>
                                    <select className="filter-select" name="tipo" value={filters.tipo} onChange={handleFilterChange}>
                                        <option value="todos">Todos</option><option value="producto">Producto</option><option value="servicio">Servicio</option>
                                    </select>
                        ...
                        <div className="vencimiento-items-grid">
                            {isLoading ? <p>Cargando...</p> : filteredAndSortedData.map(item => (
                                <div key={item.id} className={`vencimiento-item-card ${selectedProduct?.id === item.id ? 'selected' : ''}`} onClick={() => setSelectedProduct(item)}>
                                    <span className="item-name">{item.name}</span>
                                    <strong className="item-price">${(item.price || 0).toFixed(2)}</strong>
                                </div>
                            ))}
                        </div>
                    </div>
                    <div className="vencimiento-details-section">
                        ...
                        <div className="modal-footer">
                            ...
                            <button type="submit" className="btn btn-primary" disabled={isSubmitting || !selectedProduct}>{isSubmitting ? 'Guardando...' : 'Guardar'}</button>
                        </div>
                    </div>
                </form>
            </div>
