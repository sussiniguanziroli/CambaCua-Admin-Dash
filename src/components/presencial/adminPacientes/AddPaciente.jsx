  ...
  useEffect(() => {
    ...
    const fetchEspeciesYRazas = async () => {
      ...
      try {
        ...
        Swal.fire(
          "Error",
          "No se pudieron cargar las especies y razas.",
          "error"
        );
      } finally {
  ...
  const handleChange = (e) => {
    ...
    if (name === "species") {
      ...
      setPacienteData((prev) => ({
        ...prev,
        fallecido: checked,
        fechaFallecimiento: checked ? prev.fechaFallecimiento : "",
      }));
    } else {
  ...
  const handleServiceTypeChange = (e) => {
    ...
    setPacienteData((prev) => {
      ...
      if (checked) {
        ...
        return {
          ...prev,
          serviceTypes: currentServices.filter((service) => service !== value),
        };
      }
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSave = {
        ...
        serviceTypes: pacienteData.serviceTypes || [],
          ...
          ? [
              {
                ...
                diagnosis: "N/A",
                treatment: pacienteData.clinicalNotes,
              },
            ]
          : [],
      };
      ...
      <form onSubmit={handleSubmit} className="paciente-styled-form">
        <fieldset className="paciente-form-fieldset">
          ...
          <div className="paciente-form-group">
            ...
            <select
              id="tutorId"
              name="tutorId"
              value={pacienteData.tutorId}
              onChange={handleChange}
              required
              disabled={isLoadingTutores}
            >
              <option value="">
                {isLoadingTutores ? "Cargando..." : "Seleccionar Tutor"}
              </option>
              {tutores.map((tutor) => (
          ...
          <div className="paciente-form-row">
            <div className="paciente-form-group">
              ...
              <select
                id="species"
                name="species"
                value={pacienteData.species}
                onChange={handleChange}
                required
                disabled={isLoadingEspecies}
              >
                <option value="">
                  {isLoadingEspecies ? "Cargando..." : "Seleccionar Especie"}
                </option>
            ...
            <div className="paciente-form-group">
              ...
              <select
                id="breed"
                name="breed"
                value={pacienteData.breed}
                onChange={handleChange}
                disabled={!pacienteData.species}
              >
        ...
        <fieldset className="paciente-form-fieldset">
          ...
          <div className="paciente-form-row">
            <div className="paciente-form-group">
              ...
              <input
                ...
                type="number"
                step="0.1"
                value={pacienteData.weight}
                onChange={handleChange}
              />
            </div>
            <div className="paciente-form-group">
              <label htmlFor="chipNumber">N° de Chip</label>
              <input
                id="chipNumber"
                name="chipNumber"
                type="text"
                value={pacienteData.chipNumber}
                onChange={handleChange}
              />
            </div>
          </div>
          <div className="paciente-form-group">
            ...
            <div className="paciente-checkbox-group-horizontal">
              <div className="paciente-checkbox-item">
                <input
                  id="serviceClinical"
                  type="checkbox"
                  value="clinical"
                  checked={(pacienteData.serviceTypes || []).includes(
                    "clinical"
                  )}
                  onChange={handleServiceTypeChange}
                />
                <label htmlFor="serviceClinical">Clínica</label>
              </div>
              <div className="paciente-checkbox-item">
                <input
                  id="serviceGrooming"
                  ...
                  onChange={handleServiceTypeChange}
                />
                <label htmlFor="serviceGrooming">Peluquería</label>
              </div>
            </div>
          </div>
          <div className="paciente-form-group">
            ...
            >
              <input
                ...
                name="fallecido"
                type="checkbox"
                checked={pacienteData.fallecido}
                onChange={handleChange}
                style={{ width: "auto" }}
              />
          ...
          {pacienteData.fallecido && (
            <div className="paciente-form-group">
              ...
              <input
                ...
                type="date"
                value={pacienteData.fechaFallecimiento}
                onChange={handleChange}
                required
              />
            </div>
          ...
          <div className="paciente-form-group">
            <label htmlFor="clinicalNotes">Notas Iniciales</label>
            <textarea
              id="clinicalNotes"
              name="clinicalNotes"
              rows="4"
              value={pacienteData.clinicalNotes}
              onChange={handleChange}
            ></textarea>
          </div>
        </fieldset>
        <div className="paciente-form-actions">
          <button
            type="submit"
            className="paciente-form-submit-btn"
