...
const PacienteProfile = () => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const [pacienteSnap, historySnap, recipesSnap, citasSnap, vencSnap] = await Promise.all([ getDoc(pacienteRef), getDocs(historyQuery), getDocs(recipesQuery), getDocs(citasQuery), getDocs(vencQuery) ]);
  ...
  const handleSaveClinicalNote = async (formData, originalNote) => {
    try {
      if (originalNote) { await updateDoc(doc(db, `pacientes/${id}/clinical_history`, originalNote.id), formData); setAlert({ message: 'Nota clínica actualizada.', type: 'success' });
      } else { await addDoc(collection(db, `pacientes/${id}/clinical_history`), { ...formData, createdAt: Timestamp.now() }); setAlert({ message: 'Nota agregada.', type: 'success' }); }
      handleCloseModals(); await fetchAllData();
    } catch (error) { setAlert({ message: 'No se pudo guardar la nota.', type: 'error' }); }
  };
  
  const handleSaveRecipe = async (recipeData) => {
    try {
        await addDoc(collection(db, `pacientes/${id}/clinical_recipes`), { ...recipeData, createdAt: Timestamp.now() });
        setAlert({ message: 'Receta guardada.', type: 'success' });
        handleCloseModals(); await fetchAllData();
    } catch (error) { setAlert({ message: 'No se pudo guardar la receta.', type: 'error' }); }
  };

  const handleDeleteRecipe = async (recipeId) => {
    const { isConfirmed } = await Swal.fire({ title: '¿Eliminar Receta?', text: 'Esta acción no se puede deshacer.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar' });
    if (isConfirmed) {
        try { await deleteDoc(doc(db, `pacientes/${id}/clinical_recipes`, recipeId)); setAlert({ message: 'Receta eliminada.', type: 'success' }); await fetchAllData();
        } catch (error) { setAlert({ message: 'No se pudo eliminar la receta.', type: 'error' }); }
    }
  };

  const handlePrintRecipe = async (elementId) => {
    const input = document.getElementById(elementId); const canvas = await html2canvas(input, { scale: 2 }); const imgData = canvas.toDataURL('image/png');
    const pdf = new jsPDF({ orientation: 'p', unit: 'mm', format: 'a4' }); const pdfWidth = pdf.internal.pageSize.getWidth(); const pdfHeight = (canvas.height * pdfWidth) / canvas.width;
    pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight); pdf.save(`receta_${paciente.name}_${new Date().toLocaleDateString('es-AR')}.pdf`);
  };
  ...
  const filteredAndSortedHistory = useMemo(() => {
    ...
    if (historyFilters.startDate) { const start = new Date(historyFilters.startDate); start.setHours(0,0,0,0); filtered = filtered.filter(e => e.createdAt.toDate() >= start); }
    if (historyFilters.endDate) { const end = new Date(historyFilters.endDate); end.setHours(23,59,59,999); filtered = filtered.filter(e => e.createdAt.toDate() <= end); }
    if (historyFilters.searchTerm) { const term = historyFilters.searchTerm.toLowerCase(); filtered = filtered.filter(e => ((e.reason || '').toLowerCase().includes(term) || (e.diagnosis || '').toLowerCase().includes(term) || (e.treatment || '').toLowerCase().includes(term))); }
    filtered.sort((a, b) => { if (historyFilters.sortOrder === 'date-desc') return b.createdAt.toMillis() - a.createdAt.toMillis(); return a.createdAt.toMillis() - b.createdAt.toMillis(); });
    return filtered;
  }, [clinicalHistory, historyFilters]);

  const filteredAndSortedRecipes = useMemo(() => {
    let filtered = [...recipes];
    if (recipeFilters.startDate) { const start = new Date(recipeFilters.startDate); start.setHours(0,0,0,0); filtered = filtered.filter(r => r.createdAt.toDate() >= start); }
    if (recipeFilters.endDate) { const end = new Date(recipeFilters.endDate); end.setHours(23,59,59,999); filtered = filtered.filter(r => r.createdAt.toDate() <= end); }
    if (recipeFilters.searchTerm) { const term = recipeFilters.searchTerm.toLowerCase(); filtered = filtered.filter(r => (r.prescribedBy || '').toLowerCase().includes(term) || (r.prescriptions || []).some(p => (p.productName || '').toLowerCase().includes(term))); }
    filtered.sort((a, b) => { if (recipeFilters.sortOrder === 'date-desc') return b.createdAt.toMillis() - a.createdAt.toMillis(); return a.createdAt.toMillis() - b.createdAt.toMillis(); });
    return filtered;
  ...
  const renderCitas = () => (<div className="tab-content">{citas.length > 0 ? (citas.map(cita => (<div key={cita.id} className="cita-card"><p><strong>Fecha:</strong> {cita.startTime.toLocaleString('es-AR')}</p><p><strong>Servicios:</strong> {(cita.services || []).map(s => s.name || s.nombre).join(', ')}</p></div>))) : <p className="no-results-message">No hay citas.</p>}</div>);
  const renderRecetas = () => (<div className="tab-content"><div className="recipe-controls"><button className="btn btn-primary" onClick={() => setIsCreateRecipeModalOpen(true)}>+ Nueva Receta</button><div className="filters-group"><input type="text" name="searchTerm" placeholder="Buscar por doctor o producto..." value={recipeFilters.searchTerm} onChange={handleRecipeFilterChange} /><input type="date" name="startDate" value={recipeFilters.startDate} onChange={handleRecipeFilterChange} /><input type="date" name="endDate" value={recipeFilters.endDate} onChange={handleRecipeFilterChange} /><select name="sortOrder" value={recipeFilters.sortOrder} onChange={handleRecipeFilterChange}><option value="date-desc">Más Recientes</option><option value="date-asc">Más Antiguas</option></select></div></div><div className="recipe-list">{filteredAndSortedRecipes.length > 0 ? filteredAndSortedRecipes.map(recipe => (<div key={recipe.id} className="recipe-list-item"><span>Receta del {recipe.createdAt.toDate().toLocaleDateString('es-AR')}</span><div className="recipe-actions"><button className="btn btn-secondary" onClick={() => setSelectedRecipe(recipe)}>Ver/Imprimir</button><button className="btn btn-delete-small" onClick={() => handleDeleteRecipe(recipe.id)}><FaTrash/></button></div></div>)) : <p className="no-results-message">No hay recetas.</p>}</div></div>);
  ...
  return (
    <div className="profile-container paciente-profile">
      ...
      <div className="profile-header"><div className="profile-avatar">{paciente.species === 'Canino' ? <FaDog /> : <FaCat />}</div><div className="profile-info"><h1>{paciente.name}</h1><p>Tutor: <Link to={`/admin/tutores/${paciente.tutorId}`}>{paciente.tutorName}</Link></p></div><div className="profile-actions"><Link to={`/admin/edit-paciente/${id}`} className="btn btn-secondary">Editar Paciente</Link><button className="btn btn-primary" onClick={handleAddNewNote}>+ Nota Clínica</button></div></div>
      <div className="details-bar"><div className="detail-chip"><strong>Especie:</strong> {paciente.species}</div><div className="detail-chip"><strong>Raza:</strong> {paciente.breed}</div><div className="detail-chip"><strong>Sexo:</strong> {paciente.gender}</div><div className="detail-chip"><strong>Edad:</strong> {calculateAge(paciente.birthDate)}</div><div className="detail-chip"><strong>Peso:</strong> {paciente.weight ? `${paciente.weight} kg` : 'N/A'}</div><div className="detail-chip"><strong>Chip:</strong> {paciente.chipNumber || 'N/A'}</div></div>
