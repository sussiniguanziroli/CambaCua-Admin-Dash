                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {(existingMedia.length > 0 || filesToUpload.length > 0) && (
                        <div className="file-preview-list">
                            {existingMedia.map((file, i) => <div key={i} className="file-preview-item"><span>{file.name}</span><button type="button" onClick={() => removeExistingFile(file)}>&times;</button></div>)}
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="submit" className="btn btn-primary" disabled={isSubmitting}>{isSubmitting ? 'Guardando...' : 'Actualizar Nota'}</button>
                    </div>
                </form>
            </div>
        </div>
...
export default EditNotaPeluqueriaModal;